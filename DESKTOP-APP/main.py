...
def loadDataSets():
    ...
    if isRecognizeOn:
        isRecognizeOn = not isRecognizeOn
        ...
        while True:
            ...
            if usingOpenCv2:
                ...
                for (x, y, w, h) in faces:
                    ...
                    end_cord_x = x + w
                    end_cord_y = y + h
                    cv2.rectangle(cv_img, (x, y), (end_cord_x,
                ...
                if isRecognizeTempOn:
                    for (top, right, bottom, left), name in zip(face_locations, face_names):
                        ...
                        if name != "Unknown":
                            logging.info(
                                f"Employee with ID: {name} has been detected!")
                        else:
                            logging.warning(
                                f"A stranger has been detected! Saved to the warning database!")
...
class MainScreen(QtWidgets.QMainWindow):
    ...
    def choseFile(self):
        ...
        file_name = QFileDialog.getOpenFileNames(
            file_dialog, "Chose Images", "", file_filter)

        if file_name[0]:
            self.ui.txt_file_name.setPlainText(
                file_name[0][0][file_name[0][0].rfind("/"):])
    ...
    def addUser(self):
        ...
        if id != "" and self.filePath:
            ...
            QtWidgets.QMessageBox().warning(
                self, "Warning", "Please chose a image and input the ID!")
            return

        if isContinue:
            copyfile(self.filePath, app_path + f"\\images\\{id}{self.ext}")
            self.worker = WorkerTrainer()
            self.worker.start()
    ...
    def worker_finished(self):
        QtWidgets.QMessageBox().information(self, "Notification",
                                                "Add user to the dataset successfully!")
        ...
        if isRecognizeOn and face_names != []:
            ...
            for name in face_names:
                if name == "Unknown":
                    ...
                    im.save(app_path + f"\\unknows\\{uuid.uuid4().hex}.png")
                    self.ui.txt_logs.appendPlainText(
                        f"{dt_string} - Unauthorized person detected!")
                else:
                    if f"{dt_string} - Employee with ID: {name} has been detected!" not in log_text:
                        self.ui.txt_logs.appendPlainText(
                            f"{dt_string} - Employee with ID: {name} has been detected!")
...
class LoginScreen(QtWidgets.QMainWindow):
    ...
    def login(self):
        ...
        newHeaders = {'Content-type': 'application/json',
                      'Accept': 'text/plain'}
        #result = requests.post(f"{URL}/user/validate/", data=data)

        response = requests.post(f"{URL}/user/validate/",
                               json=data)
        ...
        if validated == 1:
            ...
            QtWidgets.QMessageBox.warning(
            self.ui.centralwidget, "Login", "Username or Password is incorrect!")
        elif validated == -2:
            QtWidgets.QMessageBox.warning(
            self.ui.centralwidget, "Login", "Access Denied!")
        else:
            QtWidgets.QMessageBox.warning(
            self.ui.centralwidget, "Login", "Your account has been blocked or have activated yet! No further information!")
